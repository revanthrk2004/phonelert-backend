...
def train_knn_model(user_id, force=False):  
    with app.app_context():
...
def check_location():
    ...
    data = request.json
    print("📥 Incoming check-location request:", data)  
...
def test_email():
    ...
    try:
        msg = Message(
            ...
            recipients=["adhithyahere7@gmail.com"],  
...
def cluster_user_locations(user_id):
    try:
        cluster_and_save_user_locations(user_id)
        return jsonify({"message": "✅ Zones Updated"}), 200  
    except Exception as e:
        return jsonify({"error": str(e)}), 500
